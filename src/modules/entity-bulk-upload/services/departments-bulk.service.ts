...
import { DepartmentBulkUploadResultDto, DepartmentBulkUploadResponseDto } from '../dto/department-bulk-upload.dto';
import { slugify } from '../../../common/utils/slugify';

interface DepartmentParsedRow {
  name: string;
  description?: string;
  image_url?: string;
  is_active?: string;
...
@Injectable()
export class DepartmentsBulkService {
  ...
  generateTemplate(): string {
    ...
    const exampleRows = [
      ['Oral Care', 'Products for oral hygiene and dental care', '', 'true', '1'],
      ['Personal Care', 'Personal hygiene and care products', '', 'true', '2'],
    ];
  ...
  private async parseCSV(file: Express.Multer.File): Promise<DepartmentParsedRow[]> {
    const content = file.buffer.toString('utf-8');
    return new Promise((resolve, reject) => {
      const rows: DepartmentParsedRow[] = [];
      parseString(content, { headers: true, trim: true })
        .on('data', (row: DepartmentParsedRow) => rows.push(row))
        .on('end', () => resolve(rows))
  ...
  private async uploadToImageKit(fileBuffer: Buffer, fileName: string): Promise<string> {
    ...
    formData.append('folder', '/dentalkart/departments');
  ...
  async processBulkUpload(file: Express.Multer.File): Promise<DepartmentBulkUploadResponseDto> {
    ...
    const results: DepartmentBulkUploadResultDto[] = [];
    ...
    for (const row of rows) {
      ...
      try {
        ...
        let counter = 1;
        while (await this.departmentRepository.findOne({ where: { slug: finalSlug } })) {
